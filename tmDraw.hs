...
initTape (vs, fs) = foldl' (f (vs, fs)) (Tape $ Map.empty) [0..length vs-1]
    where f :: ([Vertex3 GLfloat], [[VertexIndex]]) -> Tape -> VertexIndex -> Tape
          ...
          angle :: Vertex3 GLfloat -> Vertex3 GLfloat -> Vertex3 GLfloat -> Float
          angle (Vertex3 p1 p2 p3) (Vertex3 r1 r2 r3) (Vertex3 x1 x2 x3) = 
          
          --angle :: Vertex3 GLfloat -> Vertex3 GLfloat -> Float
          --angle (Vertex3 x1 y1 z1) (Vertex3 x2 y2 z2) = let d = x1*x2 + y1*y2 + z1*z2
          --                                                  r1 = sqrt $ x1^^2 + y1^^2 + z1^^2
          --                                                  r2 = sqrt $ x2^^2 + y2^^2 + z2^^2
          --                                                  xn = y1 * z2 - z1 * y2
          --                                                  yn = z1 * x2 - x1 * z2
          --                                                  zn = x1 * y2 - y1 * x2
          --                                                  s  = sqrt $ xn^^2 + yn^^2 + zn^^2                                                                                                                              
          --                                                  
          --                                              in --realToFrac $ acos (realToFrac d / (r1 * r2))
          --                                                 realToFrac $ atan2 s d
